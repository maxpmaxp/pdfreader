# The code is based on https://github.com/euske/pdfminer/blob/master/pdfminer/ccitt.py
...
class CCITTG4Parser(BitParser):
    WHITE = _build_white_tree()
    BLACK = _build_black_tree()
...
##  CCITTG4FaxDecoder
##
class CCITTG4FaxDecoder(CCITTG4Parser):
...
##  CCITTG3Parser
##
class CCITTG3Parser(BitParser):
    WHITE = _build_white_tree()
    BLACK = _build_black_tree()
    FILL = _build_fill_loop()

    BitParser.add(WHITE, 'eol', '000000000001')
    BitParser.add(BLACK, 'eol', '000000000001')


    def __init__(self, width, bytealign=False, is_eol_required=False):
        BitParser.__init__(self)
        self.width = width
        self.bytealign = bytealign
        self.is_eol_required = is_eol_required
        self.reset()
    ...
    def _flush_line(self):
        ...
        if len(self._curline) < self.width:
            diff = self.width - len(self._curline)
            self._curline += array.array('b', [0] * diff)
        self.output_line(self._y, self._curline)
        self._reset_line()
        if self.bytealign:
            raise ByteSkip
...
##  CCITTG3FaxDecoder
##
class CCITTG3FaxDecoder(CCITTG3Parser):

    def __init__(self, width, bytealign=False, reversed=False, is_eol_required=False):
        CCITTG3Parser.__init__(self, width, bytealign=bytealign)
        self.reversed = reversed
        self._buf = b''
...
def ccittfaxdecode(data, params):
    """
    >>> data = open("pdfreader/filters/ccitt-g3.encoded", "rb").read()
    >>> params = {'K': 0, 'Columns': 4912, 'EncodedByteAlign': True}
    ...
    is_eol_required = params.get('EndOfLine', False)
    if K < 0:
        parser = CCITTG4FaxDecoder(cols, bytealign=bytealign, reversed=reversed)
    elif K == 0:
        parser = CCITTG3FaxDecoder(cols, bytealign=bytealign, reversed=reversed, is_eol_required=is_eol_required)
    else: # K > 0
        raise ValueError("Mixed G3 2-d encoding is not supported")
